...
def main():
    ...
    for t in tiedot:
        if t[8]:  
            pvm = t[4].strftime("%d.%m.%Y")
            klo = t[5].strftime("%H.%M")
            print(f"- {t[1]}, {t[9]}, {pvm} klo {klo}")
    print()
    ...
    for t in tiedot:
        if t[6] >= 3:
            pvm = t[4].strftime("%d.%m.%Y")
            klo = t[5].strftime("%H.%M")
            print(f"- {t[1]}, {pvm} klo {klo}, kesto {t[6]} h, {t[9]}")
    print()
    ...
    for t in tiedot:
        tilanne = "Vahvistettu" if t[8] else "Ei vahvistettu"
        print(f"{t[1]} → {tilanne}")
    print()
    ...
    tulot_euro = f"{tulot:.2f}".replace(".", ",")
    print(f"Vahvistettujen varausten kokonaistulot: {tulot_euro} €")
    print()
